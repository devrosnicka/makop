...
import { SeasonReceivables } from "@/components/seasons/SeasonReceivables";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {calculation
            ? "Výpočet je uzamčený a už se nemění."
            : "Sezóna zatím nemá výpočet."}
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-6">
        {deleteCalculation.error && (
          <Alert variant="destructive">
            <AlertDescription>
              {deleteCalculation.error.message}
            </AlertDescription>
          </Alert>
        )}
        ...
        {!calculation ? (
          <Button asChild>
            <Link
              to="/seasons/$seasonId/calculation"
              params={{ seasonId: String(id) }}
            >
              Spočítat příspěvek
            </Link>
          </Button>
          ...
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <dl className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-2 text-sm">
                ...
                <dd className="text-right tabular-nums">
                  {calculation.referee_match_count}
                </dd>
              </dl>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <h3 className="text-sm font-medium">
                Hráči ve výpočtu ({players.length})
              </h3>
              <ul className="flex flex-col gap-1 text-sm">
                {players.map((player) => (
                  <li
                    key={player.id}
                    className="rounded-md border bg-card px-3 py-2 break-words"
                  >
            ...
            {/* Deleting the calculation is refused once it has produced
                receivables (they'd be left pointing at nothing), so the
                affordance goes away rather than failing on click. */}
            {receivablesCount > 0 ? (
              <p className="text-xs text-muted-foreground">
                Výpočet už nejde smazat — nejdřív by bylo potřeba odstranit
                vygenerované pohledávky.
              </p>
            ) : (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      Smazat výpočet sezóny {season.name}?
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      Uzamčený výpočet nejde upravit — smazáním zmizí i snapshot
                      hráčů a bude potřeba spočítat příspěvek znovu.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Zrušit</AlertDialogCancel>
                    <AlertDialogAction
                      onClick={() => void deleteCalculation.mutateAsync()}
                    >
                      Smazat
                    </AlertDialogAction>
                  </AlertDialogFooter>
